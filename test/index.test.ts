...
describe('useXprovider', () => {
  ...
  it('should work with partical types', () => {
    ...
    mount(
      defineComponent({
        render() {
          return h(providerFoundation.ProviderComponent, {
            scopedSlots: { default: () => h(descendNode) },
          })
        },
      }),
    )
  ...
  it('should support type-safe string dot notation', () => {
    ...
    const cars = [{ brand: 'byd' }] as unknown as ICar[]
    const person = {
      name: 'phillyx',
      age: 20,
      action: () => {
